...
#' xmat <- cbind(rt(n_subj*T_obs, df=2, ncp=1.3), 1.2*fvec +
...
erfeVecR <- function(xmat, yvec, panSizeVec, asym, id){
  ...
  while (dw1 != 0 && it < 50) {
    ...
    # betaEstimate = solve(diagmat(sqrt(w1))*xdexpect, diagmat(sqrt(w1))*ydexpect)
    
    betaEstimate <- stats::lsfit(xdexpect, ydexpect, wt=w1, intercept = FALSE)$coef	
  ...
  mat_meat <- t(xmat)%*%bdiag_sigma%*%xmat # diag((c(w1)*res)^2)
  dof <- length(weight_res)/(length(weight_res) -  ncol(xmat) - length(split_id))
  cov_mat <- (dof*mat_bread)%*%mat_meat%*%mat_bread
  ecart_type <- sqrt(diag(as.matrix(cov_mat)))
  
  out <- list(asymPoint=asym, coefEst = betaEstimate, standardError=ecart_type,
              asymWeight = c(w1), Residuals=res, covMat=cov_mat)
